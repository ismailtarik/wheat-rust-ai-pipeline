...
def run_detection_phase(split_result: dict, config: dict,
                         ...
                         data_yaml_override: str = None) -> dict:
    """
    Pipeline complet de la Phase 2 (Detection) :
        1. Conversion du dataset vers le format YOLO (bbox = full_image),
           SAUF si data_yaml_override est fourni (vraies bboxes déjà prêtes)
        2. Entrainement YOLO
        3. Evaluation (mAP, precision, recall -- globale et par classe)
        4. Sauvegarde des resultats et figures
    ...
    Args:
        ...
        skip_existing : si True et qu'un modele entraine existe deja
                        (best.pt + metrics.json), le recharge sans
                        re-entrainer (utile apres une coupure de session)
        data_yaml_override : chemin vers un data.yaml YOLO déjà construit
                        avec de VRAIES bounding boxes (ex: dataset fusionné
                        Roboflow). Si fourni, la conversion bbox=full_image
                        est ignorée et ce fichier est utilisé directement.
    ...
    # 1. Dataset YOLO — vraies bboxes si fournies, sinon conversion full_image
    if data_yaml_override:
        print(f"\n  Utilisation de bboxes réelles (pas de conversion) :")
        print(f"  {data_yaml_override}")
        data_yaml_path = data_yaml_override
    else:
        data_yaml_path = build_yolo_dataset(split_result, config)
